...
fun OverviewScreen(
    ...
    LazyColumn(modifier = modifier.fillMaxSize()) {
        uiState.yearPhotos.forEach { year ->
            ...
            uiState.monthPhotos.filter{it.first == year.first}.filter { it.third != 0 }.forEach { month ->
                item {
                    MonthRow(month.second.toDutchString(), month.third, onClick )
                }
            }
        }
    }
...
fun MonthRow(month: String, nrOfFotos: Int, onClick : () -> Unit){
    Row(
        modifier = Modifier.padding(vertical = 15.dp).clickable {
            onClick()
        },
